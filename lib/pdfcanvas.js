...
(function () {
    ...
    function addImage(image, images) {
        ...
        if (image.hash) {
            Object.assign(meta, image);
        } else if (image.data) {
            meta.width = image.width;
            meta.height = image.height;
            meta.type = 'buf';
            meta.hash = makeHash(image.data);

            saveBuf = image.data;
        } else if (image.src) {
            const pos = image.src.indexOf(',');

            if (pos >= 0) {
                const buf = new Buffer(image.src.substr(pos+1), 'base64');
                const dim = sizeOf(buf);

                meta.width = dim.width;
                meta.height = dim.height;
                meta.type = dim.type;
                meta.hash = makeHash(buf);
        ...
        function createImagePath(dir, meta) {
            const prefix = path.join(dir, meta.hash);

            if (meta.type === 'buf') {
                return prefix + '-' + meta.width + 'x' + meta.height + '.' + meta.type;
            } else {
                return prefix + '.' + meta.type;
            }
        }

        if (_exportImageDir && saveBuf) {
            const imagePath = createImagePath(_exportImageDir, meta);

            if (!fs.existsSync(imagePath)) {
                fs.writeFile(imagePath, saveBuf, function (err) {
                    if (err) {
                        nodeUtil.p2jWarn('Could not write ' + imagePath);
                    }
                });
            }
        }
    ...
    CanvasRenderingContext2D_.exportImagesToDir = function exportImagesToDir(d) {
        _exportImageDir = d;
    }
